...
export default class IdentityReflection extends Reporter(EventEmitter) {
    ...
    async useIdentity(settings) {
        await this.connectSE();
        await me.hosted(settings);
        me.alias = settings.alias;
        return me;
    }
    ...
    async getSecretWorker() {
        return universe.GET_SECRET_WORKER
                ...
                : await Facade.use(await WorkerProvider.from('/thoregon.identity/lib/identitysecret.mjs'));
    }
    ...
    async connectSE() {
        ...
        try {
            ...
            if (!globalThis.me) universe.global('me', identity);              // caution: can't be changed later. secure all methods by checking if user is signed on
        } catch (e) {
