...
export default class IdentityReflection extends Reporter(EventEmitter) {
    ...
    static async getIdentiyClass() {
        const CLASS_REF = universe.IDENTITY_CLASS ?? './identity.mjs';
    ...
    migrateAnchor(anchor) {
        if (universe.neuland.has(universe.neuland.ROOT)) return universe.neuland.ROOT;
        if (!universe.neuland.has(anchor)) {
            // create new SSI
        } else {
            // migrate DB entry
        }
