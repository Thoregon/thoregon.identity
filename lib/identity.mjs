    ...
    async ssi() {
        if (!this._ssi) {
            ...
            if (existsLocal) {
                ...
                if (!ssi && universe.DEV) {
                    // DON'T recreate automatically, also not in DEV. use 'universe.restoreSSI()' if needed
                    // ssi = await this.restoreSSI(anchor, born);
                }
            }
            this._ssi = ssi;
            if (!ssi) {
                ...
                //     this._ssi = ssi;
                // } else {
                    console.log(">> SSI could not be established");
                    return;
                //}
            }
