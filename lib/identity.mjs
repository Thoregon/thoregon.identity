...
import ThoregonObject         from "/thoregon.archetim/lib/thoregonobject.mjs";
...
// GunIdentityResolver --> SSI_RESOLVER
...
/*
            const ssi = await ThoregonObject.materialized()
                        ? SelfSovereignIdentity.from(anchor)
                        : SelfSovereignIdentity.create({}, { store: anchor });
            this._ssi = ssi;
*/
        }
        return this._ssi;
...
/*
    async device() {
        if (!this._device) {
            const deviceid = universe.t͛device;
            ...
            if (!device) {
                ...
                await doAsync();
                device = devices[deviceid];     // get the wrapped device
            }
            this._device = device;
        }
...
/*
    async devices() {
        if (!this._devices) {
            const ssi = await this.ssi();
            let devices = await ssi.devices;
            if (!devices) {
                devices = new Collection();
                ssi.devices = devices;
                await doAsync();
                devices = await ssi.devices;        // get the wrapped collection
            }
