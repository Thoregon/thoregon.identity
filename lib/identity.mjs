...
export default class Identity extends SharedCrypto(Reporter()) {
    ...
    async restoreSSI(anchor, born = false) {
        ...
        ssi._created = false;
        ssi.demandSync();
        if (!born) await timeout(born ? SSI_SYNC_WAIT : SSI_SYNC_FOREIGN_WAIT); // this should be enough to let the SSI sync
        if (!ssi.materialized) {
    ...
    */
    ...
    async hosted(settings, opt) {
        this.isHosted = true;
        await this.SE.hosted( { pairs: settings.pairs }, settings.salt, { id: settings.id, anchor: settings.anchor });
        await this.ssi({ hosted: true });
    }
